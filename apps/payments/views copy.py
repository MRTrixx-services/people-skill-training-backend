...
class PaymentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = Payment.objects.filter(user=self.request.user).select_related('platform')
...
#     """
#     Checkout endpoint for multiple selected webinar items.
#     Returns consistent response structure for frontend.
#     """
...
#     logger.info(f"  Items: {len(selected_items)}")
#     logger.info(f"  Items data: {selected_items}")

#     # Validate payment method
#     if not payment_method:
#         logger.error("❌ Payment method missing")
#         return Response({
#             'success': False,
...
#         return Response({
#             'success': False,
#             'error': 'Selected items list is required',
#             'error_code': 'INVALID_ITEMS'
#         }, status=status.HTTP_400_BAD_REQUEST)

#     # Extract and validate webinar IDs
#     webinar_ids = list(set([item.get('webinarId') for item in selected_items if 'webinarId' in item]))
#     if not webinar_ids:
#         logger.error("❌ No webinar IDs")
#         return Response({
#             'success': False,
#             'error': 'No valid webinar IDs found in items',
#             'error_code': 'NO_WEBINAR_IDS'
#         }, status=status.HTTP_400_BAD_REQUEST)
    
#     webinars = list(Webinar.objects.filter(id__in=webinar_ids))
#     existing_ids = set(w.id for w in webinars)
#     missing_ids = set(webinar_ids) - existing_ids
#     if missing_ids:
#         logger.error(f"❌ Missing webinars: {missing_ids}")
#         return Response({
#             'success': False,
#             'error': f'Webinars not found: {list(missing_ids)}',
#             'error_code': 'WEBINARS_NOT_FOUND',
...
#             logger.error(f"❌ Invalid price: {item}")
#             return Response({
#                 'success': False,
#                 'error': f'Invalid price for item: {item}',
...
#     # Validate Gateway
#     try:
#         gateway = PaymentGateway.objects.get(gateway_id=payment_method, is_active=True)
#         logger.info(f"✅ Gateway found: {gateway.display_name}")
#         logger.info(f"  Is configured: {gateway.is_configured}")
        
#         # Check configuration
#         if not gateway.is_configured:
#             if gateway.is_test_mode:
#                 logger.warning(f"⚠️ {gateway.display_name} test mode - proceeding")
#                 if not gateway.configuration:
#                     gateway.configuration = {
#                         'client_id': 'test_client_id',
#                         'client_secret': 'test_client_secret'
#                     }
#             else:
#                 logger.error("❌ Gateway not configured for production")
#                 return Response({
#                     'success': False,
#                     'error': f'{gateway.display_name} is not properly configured',
#                     'error_code': 'GATEWAY_NOT_CONFIGURED',
#                     'details': 'Please configure payment gateway credentials in admin panel'
#                 }, status=status.HTTP_400_BAD_REQUEST)
        
#         # Validate amount range
#         if total_amount < gateway.min_amount or total_amount > gateway.max_amount:
#             logger.error(f"❌ Amount out of range: {gateway.min_amount} - {gateway.max_amount}")
#             return Response({
#                 'success': False,
...
#         logger.error(f"❌ Gateway validation error: {str(e)}", exc_info=True)
#         return Response({
#             'success': False,
#             'error': f'Gateway validation failed: {str(e)}',
#             'error_code': 'GATEWAY_VALIDATION_ERROR'
#         }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

#     # Get platform from request
#     platform = getattr(request, 'platform', None)

#     # ✅ FIX: Create payment record with proper through table handling
#     try:
...
#             # ✅ Group items by webinar ID
#             webinar_items_map = {}
#             for item in selected_items:
#                 webinar_id = item['webinarId']
#                 if webinar_id not in webinar_items_map:
#                     webinar_items_map[webinar_id] = []
#                 webinar_items_map[webinar_id].append(item)

#             # ✅ Create PaymentWebinar entries directly (NOT using .add())
#             for webinar_id, items in webinar_items_map.items():
#                 webinar = next((w for w in webinars if w.id == webinar_id), None)
#                 if webinar:
#                     for item in items:
#                         # ✅ Create through table entry with amount
#                         PaymentWebinar.objects.create(
#                             payment=payment,
#                             webinar=webinar,
#                             amount=Decimal(str(item['price'])),  # ✅ Convert to Decimal
#                             access_type=item.get('itemType', 'liveOne')
#                         )
#                         logger.info(f"  ✅ Created PaymentWebinar: {webinar.title} - {item.get('itemType')} - ${item['price']}")
...
#         gateway_config = gateway.configuration or {}
#         try:
#             client = razorpay.Client(auth=(
#                 gateway_config.get('key_id', 'test_key'),
#                 gateway_config.get('key_secret', 'test_secret')
#             ))
...
#                     'webinar_ids': ','.join(map(str, webinar_ids)),
#                 }
#             }
...
#             # Build webinar details
#             webinar_details = []
#             for webinar_id in webinar_items_map.keys():
#                 webinar = next((w for w in webinars if w.id == webinar_id), None)
#                 if webinar:
#                     webinar_total = sum(item['price'] for item in webinar_items_map[webinar_id])
#                     webinar_details.append({
#                         'id': webinar.id,
#                         'title': webinar.title,
#                         'price': float(webinar_total),
#                         'instructor': webinar.speaker.user.get_full_name() if webinar.speaker and webinar.speaker.user else 'TBD',
#                         'items': [item.get('itemType') for item in webinar_items_map[webinar_id]]
#                     })
...
#                 'currency': currency,
#                 'gateway_key': gateway_config.get('key_id'),
#                 'gateway_config': {
#                     'name': gateway.display_name,
#                     'test_mode': gateway.is_test_mode,
...
#             payment.gateway_order_id = f'PAYPAL_{payment.transaction_id}'
#             payment.save()

#             logger.info(f"✅ PayPal checkout prepared: {payment.gateway_order_id}")
...
#                     'name': gateway.display_name,
#                     'test_mode': gateway.is_test_mode,
#                     'client_id': gateway_config.get('client_id', 'test_client_id'),
...
#         except Exception as e:
#             logger.error(f"❌ PayPal response error: {str(e)}", exc_info=True)
#             return Response({
#                 'success': False,
#                 'error': f'PayPal processing failed: {str(e)}',
...
#         return Response({
#             'success': False,
#             'error': f'Payment method {payment_method} not implemented yet',
#             'error_code': 'PAYMENT_METHOD_NOT_IMPLEMENTED'
        ...
        if verification_result.get('verified'):
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    # ✅ Get admin BCC list
                    # bcc_list = []
                    # if platform and platform.support_email:
                    #     bcc_list.append(platform.support_email)
                    #     logger.info(f"📧 Will send BCC copy to admin: {platform.support_email}")
...
#                         'title': pw.webinar.title,
#                         'instructor': pw.webinar.speaker.user.get_full_name() if pw.webinar.speaker else 'TBA',
#                         # ✅ FIX: Use scheduled_date instead of start_time
#                         'scheduled_date': pw.webinar.scheduled_date.strftime('%B %d, %Y at %I:%M %p') if pw.webinar.scheduled_date else 'On Demand',
#                         'duration': f"{pw.webinar.duration} minutes" if pw.webinar.duration else 'Self-paced',
#                         'webinar_url': webinar_url,
#                     })
...
#                 # Create enrollments for all webinars
#                 # Create enrollments for all webinars
#                 payment_webinars = PaymentWebinar.objects.filter(payment=payment)
...
# @permission_classes([permissions.IsAuthenticated])
# def verify_payment(request):
#     """Verify payment and create enrollments for all webinars"""
    
#     payment_id = request.data.get('payment_id')
#     order_id = request.data.get('order_id')
#     signature = request.data.get('signature')
#     gateway_method = request.data.get('payment_method', 'razorpay')
#     paypal_order_id = request.data.get('paypal_order_id')
#     payer_id = request.data.get('payer_id')

#     logger.info(f"🔍 VERIFY PAYMENT REQUEST:")
#     logger.info(f"  User: {request.user.email}")
#     logger.info(f"  Payment Method: {gateway_method}")
#     logger.info(f"  Payment ID: {payment_id}")
#     logger.info(f"  Order ID: {order_id}")
#     logger.info(f"  PayPal Order ID: {paypal_order_id}")
    
#     # Handle PayPal differently
...
#                     'error_code': 'PAYPAL_ORDER_ID_MISSING'
#                 }, status=status.HTTP_400_BAD_REQUEST)
            
#             # Find the most recent pending PayPal payment
...
#             if not payment:
#                 logger.error(f"❌ No pending PayPal payment found for user {request.user.id}")
#                 return Response({
#                     'success': False,
#                     'error': 'No pending PayPal payment found',
#                     'error_code': 'PAYMENT_NOT_FOUND'
#                 }, status=status.HTTP_404_NOT_FOUND)
            
#             logger.info(f"✅ Found payment: {payment.transaction_id}")
            
#             with transaction.atomic():
#                 # Update payment
#                 payment.gateway_payment_id = paypal_order_id
#                 payment.gateway_order_id = f'PAYPAL_{paypal_order_id}'
#                 payment.status = 'completed'
#                 payment.completed_at = timezone.now()
#                 payment.save()
                
#                 logger.info(f"✅ Payment updated to completed")
...
#                 # Create enrollments
#                 for payment_webinar in payment.payment_webinars.all():
#                     enrollment, created = Enrollment.objects.get_or_create(
#                         user=request.user,
#                         webinar=payment_webinar.webinar,
#                         defaults={
#                             'status': 'enrolled',
#                             'payment_amount': payment_webinar.amount,
#                             'payment_method': payment.payment_method,
#                             'transaction_id': payment.transaction_id,
#                             'access_type': payment_webinar.access_type,
#                             'enrolled_at': timezone.now()
#                         }
#                     )
                    
#                     if not created:
#                         existing_types = set(enrollment.access_type.split(','))
#                         existing_types.add(payment_webinar.access_type)
#                         enrollment.access_type = ','.join(sorted(existing_types))
#                         enrollment.save()
                    
#                     if created:
#                         enrollments_created.append(enrollment)
                    
#                     if payment_webinar.webinar.title not in webinars_enrolled:
#                         webinars_enrolled.append(payment_webinar.webinar.title)
...
#                         order_id=payment.id
#                     )
#                     logger.info(f"✅ Payment confirmation email queued for user {request.user.email}")
#                 except Exception as email_error:
#                     logger.error(f"❌ Failed to queue payment confirmation email: {email_error}")
                
#                 logger.info(f"✅ Created {len(enrollments_created)} enrollments")
                
#                 return Response({
#                     'success': True,
#                     'message': 'PayPal payment verified and enrollments created',
#                     'payment_id': str(payment.id),
#                     'transaction_id': payment.transaction_id,
#                     'enrollments_created': len(enrollments_created),
#                     'webinars_enrolled': webinars_enrolled,
#                     'total_amount': float(payment.amount),
#                     'currency': payment.currency
#                 })
                
#         except Exception as e:
#             logger.error(f"❌ PayPal verification error: {str(e)}", exc_info=True)
...
#                 'error_code': 'PAYPAL_VERIFICATION_FAILED',
#                 'details': str(e)
#             }, status=status.HTTP_400_BAD_REQUEST)
...
#         logger.info(f"✅ Found payment: {payment.transaction_id}")
#     except:
#         logger.error(f"❌ Payment not found with order_id: {order_id}")
#         return Response({
#             'success': False,
...
#     if gateway_method == 'razorpay':
#         try:
#             gateway = PaymentGateway.objects.get(gateway_id='razorpay', is_active=True)
#             gateway_config = gateway.configuration or {}
            
#             client = razorpay.Client(auth=(
#                 gateway_config.get('key_id'),
#                 gateway_config.get('key_secret')
#             ))
...
#                 return Response({
#                     'success': True,
#                     'message': 'Payment verified and enrollments created successfully',
#                     'payment_id': str(payment.id),
#                     'transaction_id': payment.transaction_id,
#                     'enrollments_created': len(enrollments_created),
#                     'webinars_enrolled': webinars_enrolled,
...
#             return Response({
#                 'success': False,
#                 'error': 'Razorpay gateway not configured',
#                 'error_code': 'GATEWAY_NOT_CONFIGURED'
#             }, status=status.HTTP_400_BAD_REQUEST)
#         except Exception as e:
#             payment.status = 'failed'
#             payment.failure_reason = str(e)
#             payment.save()
#             logger.error(f"❌ Razorpay verification error: {str(e)}", exc_info=True)
#             return Response({
#                 'success': False,
#                 'error': 'Payment verification failed',
...
#         return Response({
#             'success': False,
#             'error': f'Payment verification for {gateway_method} not implemented yet',
#             'error_code': 'VERIFICATION_NOT_IMPLEMENTED'
#         }, status=status.HTTP_501_NOT_IMPLEMENTED)
...
class RefundRequestListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # Order by requested_at (FIXED - was created_at)
