...
class Webinar(models.Model):
    ...
    recording_data = models.JSONField(
        default=dict,
        blank=True,
        help_text="Complete Zoom recording metadata (recording_id, duration, file info, URLs, etc.)"
    )
...
# Add these methods to your Webinar model after get_applicable_prices()
    ...
    @property
    def recording_links(self):
        """Get recording links from recording_data, Zoom recordings, or direct URL"""
        recordings = []
        
        # Priority 1: Use recording_data if available (from selected recording)
        if hasattr(self, 'recording_data') and self.recording_data and isinstance(self.recording_data, dict):
            # Extract data from the stored recording_data
            recording_entry = {
                'title': self.recording_data.get('topic') or f"Recording - {self.title}",
                'url': self.recording_data.get('play_url') or self.recording_data.get('share_url') or self.zoom_url,
                'type': 'primary_recording',
                'file_type': self.recording_data.get('file_type', 'URL'),
                'duration': self.recording_data.get('duration', self.duration if self.duration else 0),
                'size': self.recording_data.get('file_size_mb', 0),
                # Additional metadata
                'recording_id': self.recording_data.get('recording_id'),
                'zoom_meeting_id': self.recording_data.get('zoom_meeting_id'),
                'file_extension': self.recording_data.get('file_extension'),
                'recording_type': self.recording_data.get('recording_type'),
                'recorded_date': self.recording_data.get('recorded_date'),
                'recorded_time': self.recording_data.get('recorded_time'),
                'download_url': self.recording_data.get('download_url'),
            }
            recordings.append(recording_entry)
            return recordings
        
        # Priority 2: Get from Zoom meetings (for live webinars with recordings)
        if hasattr(self, 'zoom_meeting_rel') and self.zoom_meeting_rel:
            for recording in self.zoom_meeting_rel.recordings.all():
                recordings.append({
                    'title': f"{recording.recording_type} - {recording.topic}",
        ...
        # Priority 3: Get from Zoom webinars
        if hasattr(self, 'zoom_webinar_rel') and self.zoom_webinar_rel:
            for recording in self.zoom_webinar_rel.recordings.all():
                recordings.append({
                    'title': f"{recording.recording_type} - {recording.topic}",
        ...
        # Priority 4: Fallback to direct zoom_url (backward compatibility)
        if not recordings and self.zoom_url:
            recordings.append({
                'title': f"Recording - {self.title}",
                'url': self.zoom_url,
                'type': 'primary_recording',
